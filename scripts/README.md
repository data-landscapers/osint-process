...
| `lint-docs.py` | `LINT.md` #41 | Every root, `documentation/` and `wiki/*.md` file names its reader (first-line `<!-- reader: …; type: … -->`) and stays under the cap in `CLAUDE.md` → *Documentation caps*. Exit 1 on a breach or a missing reader, 2 without the cap table; `--report` lists and exits 0. Delivered by CORPUS (`X:\prepared\R87\`), the same script CORPUS runs (strategic review 5 R86–R88). |
| `page-length.py` | `WIKI-SYNC.md` → *A landing page over its line* | Lint #8's measure of one page (`measure_page()`, imported): `OVER` (exit 1), `under` or `ruled`. A Phase B slice runs it on its landing page before writing; `OVER` makes the write a synthesis rewrite, and `--trimmed PAGE --before N` records before/after words to `logs/phaseb-trims.csv` under a lock (strategic review 5 R81; R83 sums it). |
| `stage-check.py` | `SWEEP-CYCLE.md` → *Commit at boundaries* | Staged `new/` candidates at the sweep commit: `ingested:` present, required keys, `topics:`/`places:` vocabulary, sweep notes in a body, `full` over a truncated body. `--apply` fixes the mechanical three (removes `ingested:`, truncates or drops topics, sets `excerpt`); the rest are reported (strategic review 5 R84). |
...
| `cycle-manifest.py` | `SWEEP-CYCLE.md`, `SWEEP-BULLETIN.md` — the close; `INGEST.md`'s close calls `--stamp` | Writes `cycle-manifest.json`, the one file CORPUS reads about a run: the commit the mirror carries, the collection window from `logs/collection-stamp.json` (written by `--stamp`, whatever pass most recently closed ingest), the rotation's newest close, the counts the pass passed in, with `--drops` (schema 3, the sweep cycle only) the night's drops per sweep per code and the ingest drop rate per sweep, with `--hygiene` jobs registered and Phase B words trimmed since the previous manifest, and — schema 2, with `--usage`, the sweep cycle only — the night's per-stage plan-usage readings from `logs/usage-stages.jsonl` as a `usage` block keyed by stage. Git-ignored and written **after** the final commit, so `head` names the commit that is actually on `O:\`. `--check` asserts it names the local HEAD; counts are given, never inferred. Retires CORPUS's forensic parsing of OSINT's logs (strategic review task 14). Both files are gitignored and therefore unrestorable if removed — see the `.gitignore` warning above them and notes-for-osint 133, which cost a night's cycle to a stray deletion. |
| `drop-digest.py` | `SWEEP-CYCLE.md` → *What every sub-agent prompt carries* | Prints each sweep's three largest ingest-drop classes over the last rotation, from `sweep/ingest/drop-log-*.csv`, grouped as the manifest's `drops` block groups them; the parent pastes it verbatim at the top of the sweep brief (strategic review 5 R72). Read-only. |
...
| `hub-line-partition.py` | `wiki/ingest-judgment.md` §4 | Why each `raw/` source carries no `hub_line`, split by rule. **`--uncited` is the reading that matters**: `RESIDUAL-UNCITED` is the sources reaching neither a hub nor a page, and it is 0 (job 68, 2026-08-22) — non-zero means a run admitted one and wrote it nowhere. |
...
| Script | Called by | What |
|---|---|---|
| `archive-budget-docs.py` | *(no caller since `BUDGET-EXTRACT.md` retired, R57)* | Sets the four manifest columns and moves artefact + companion into `budget-archive/{ISO3}/{FY}/`, for **any** country, from a small scope CSV the pass writes. Replaced four per-country copies. Tested: `scripts/tests/test-archive-budget-docs.py`. |
| `ocr-pdf.py` | by hand, on any image-only PDF | OCR an image-only PDF to pdftotext-compatible text. |
