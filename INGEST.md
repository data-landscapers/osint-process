...
**Budget documents** (estimates volumes, appropriation acts, budget annexes, outturn and audit reports) are admitted as **companion source records** (`source_tier: budget-document`), like any source. A bare budget artefact arriving without one is catalogued on the spot, per `BUDGET-COLLECT.md` steps 2–3: the artefact goes to `budget-archive/{ISO3}/{FY}/` with its `new-budget/manifest.csv` row, and a companion record is admitted for it. `new-budget/` receives nothing from ingest.
...
**When slices run in parallel, only a register with a locking appender is written by a slice — today that is `logs/sweep-url_log.md`, through `url-log-append.py`.** Every other shared register — `lookups/raw-url-index.csv`, `lookups/artefact-md5-index.csv`, `logs/ingest-pending-writes.md`, `reviews/acquisitions.md` — is written by the parent alone: a slice writes the rows it owes into a file of its own (named for the slice, in a directory no sibling uses, alongside its scratch files) and returns the path, and the parent appends them at the merge, before Phase B. An appender with no lock, or one that rewrites the whole file, loses a sibling's rows under concurrency however carefully each slice appends, and a slice-unique file survives a session that dies before the merge, so the resume merges it rather than re-deriving it. Run alone, a slice writes the registers directly as step 11 says.
...
**Staged `body_completeness:` is a guess of the same kind, and so is the body's identity.** Before the record is written, read the body against its own title. If it is a different document, site navigation or boilerplate, or a headline-only stub, it is not the source, whatever the flag says. If the record's own `capture_note:` records truncation, it is not `full`. A fetch-failure note is not an `excerpt`: an excerpt is a verbatim portion of the document, and a note carrying facts from anywhere else puts unsourced claims into `raw/`. A page staged as `excerpt` that links its own full document gets that document fetched, per the paragraph below.
...
**Append its deltas to `logs/ingest-pending-writes.md`**, one line per affected **subject**, naming the source slug and what Phase B is to write; places contribute nothing. Its existence means Phase B is outstanding. **The row has exactly one shape — `` - `subject` — [[slug]] — what to write `` — with em-dash separators**, because `scripts/status.py` and `WIKI-SYNC.md` count only that shape and read any other as an empty queue. A slice prompt quotes the shape verbatim rather than describing it.

**Write its `logs/sweep-url_log.md` line here, at disposition, for all four** (`python scripts/url-log-append.py {admitted|dropped|contradiction|acquisition} URL`) and for an admission `python scripts/raw-url-index.py --append URL FILE PUBLISHED`, after the file is in `raw/` and before the candidate leaves `new/`. **An admission that carries a binary artefact also runs `python scripts/artefact-md5-index.py --append <artefact path>`** — the URL index cannot see a document served from two routes, and the bytes are the one thing that can (job 106; lint #39 reports what the append missed). Never batch these to the close. In a parallel run every row here except the url-log line goes to the slice's own file for the parent's merge (*The shape, the slicing*).
