...
| `cycle-manifest.py` | `SWEEP-CYCLE.md`, `SWEEP-BULLETIN.md` — the close; `INGEST.md`'s close calls `--stamp` | Writes `cycle-manifest.json`, the one file CORPUS reads about a run: the commit the mirror carries, the collection window from `logs/collection-stamp.json` (written by `--stamp`, whatever pass most recently closed ingest), the rotation's newest close, and the counts the pass passed in. Git-ignored and written **after** the final commit, so `head` names the commit that is actually on `O:\`. `--check` asserts it names the local HEAD; counts are given, never inferred. Retires CORPUS's forensic parsing of OSINT's logs (strategic review task 14). Both files are gitignored and therefore unrestorable if removed — see the `.gitignore` warning above them and notes-for-osint 133, which cost a night's cycle to a stray deletion. |
...
| `assert-containment.py` | `SWEEP-CYCLE.md`, before each stage commit | The stage's write-set asserted against `git status`, with an absolute deny set on `CLAUDE.md`, the `wiki/` specs (`reference.md` and the five it indexes) and every root procedure. Exit 1 = do not commit as it stands. |
...
| `ingest-lane.py` | `INGEST.md` → *Two lanes*, `UPDATE-WIKI.md` | Which lane each candidate in `new/` takes, off `sweep_batch:` — `status-acquire-*` and `progress-filler-*` are the backfill lane, everything else is news. A whitelist, so an unrecognised producer costs a full-price run rather than a silent skip. `--lane backfill` prints just that list, for slicing. Assigns the lane; only `update wiki backfill` opens it. |
| `ingest-merge-spool.py` | ingest, when run in slices | Merges slice spool files into the canonical logs. |
| `url-log-append.py` | sweeps and ingest | Writes one disposition line into `logs/sweep-url_log.md`, normalised. |
...
| `usage-log.py` | `SWEEP-CYCLE.md`, first act and before the last mirror | Inserts `Date`, `Time (UTC)`, `7d usage`, `5h usage` (weekly and 5-hour plan limits used, %) at the top of `logs/usage-log.csv`, newest first, read from the endpoint `/usage` uses. Never blocks; a failed read writes `n/a`. |
