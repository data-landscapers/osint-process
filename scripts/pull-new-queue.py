...
**The lane.** `scripts/ingest-lane.py` reads the lane off `sweep_batch:`. A `.md` candidate that carries no `sweep_batch:`, in a folder named with a backfill prefix (`status-acquire-`, `progress-filler-`, `dataset-`), gets `sweep_batch: <folder>-<YYYY-MM-DD>` inserted as the first frontmatter key — the prefix from the folder name, exactly as the register line says. A candidate that already carries one keeps it, and a folder with any other name adds nothing, so its items take the news lane at full price: the whitelist's own default.
