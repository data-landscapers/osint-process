...
| `usage-log.py` | `SWEEP-CYCLE.md`, first act, every stage boundary, and before the manifest | Inserts `Date`, `Time (UTC)`, `7d usage`, `5h usage` (weekly and 5-hour plan limits used, %) at the top of `logs/usage-log.csv`, newest first, read from the endpoint `/usage` uses. `--stage NAME` writes the reading to the git-ignored buffer `logs/usage-stages.jsonl` instead, to one decimal place; `--csv` writes both from one reading; `--reset` empties the buffer first. Never blocks; a failed read writes `n/a` (CSV) or `null` (buffer). |
