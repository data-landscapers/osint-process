...
- **One event, several accounts**: one carrier, the rest in its `hub_line_sources:`; **after the last slice and before `HUB-COMPILE.md`**, group the run's new `hub_line` sources by place + date + event and collapse each group.
