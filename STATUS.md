...
- **rule-candidates** — open lines in `reviews/rule-candidates.md`, the queue `RULES.md` drains: `grep -cE '^- ' reviews/rule-candidates.md`. The pass runs as the cycle's last stage (`SWEEP-CYCLE.md`), so this reads as the **open** queue — cases still under three occurrences and under 21 days old — not as a backlog waiting on a trigger. **A number that climbs is a case recurring**, which is what it is there to show.
