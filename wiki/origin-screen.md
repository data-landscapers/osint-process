...
**A row's own `rule` column overrides the generic action, and where it rules the origin admissible the item is admitted and no promotion follows.** The rows this reaches are the ones written as *how to handle this domain* rather than *this domain misreports* — a mirror of official text, a national regulator, a repository of deposited datasets, a CDN alias to be resolved to the canonical publisher host, an outlet admissible for ministry-event reports and nothing else. Reading `watch` as blacklist-on-sight against such a row would screen out primaries, which is the opposite of what the row was written to do. **Where the rule column rules a class in, the generic action does not apply to it**; where the row genuinely records a hostile shape, `watch` promotes to `drop` as above.
...
Match on the **registrable domain**, subdomains included, **and then on each parent suffix of it**, so a row written against a whole platform still catches its tenants. The registrable domain keeps one label more under a multi-tenant suffix — `vault_lib.MULTI_TENANT`, beside `SLD` — because a shared host is not a publisher: one tenant adjudicated `drop` must not condemn every other, and an unrelated tenant admitted once must not make the next one read `KNOWN`. `vault_lib.listed_domain()` is the single implementation of both halves and every gate calls it. The list is a *by-origin* list, not a quality bar: a merely weak or thin outlet never goes on it. Reserve it for origins that **misreport** — fabricate, rewrite without attribution, or launder someone else's reporting under their own byline.
