...
- **`sovereignty`**, **`colonialism`** — the retired lens values (`facets.md` §1, retired 2026-09-08). They are no longer a facet and no longer written anywhere, but the words remain wikilinked in legacy body prose and have no page and are never going to get one. **Write the word, or name the concept page that actually carries the argument.** The existing ones are left alone — never cleared in passing, and never registered as a job (`schemas.md` §4).
