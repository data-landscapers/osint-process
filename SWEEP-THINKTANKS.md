...
# Sweep — thinktanks (procedure)

**Content-scoped, not time-scoped** (unlike the daily sweep): works the think-tank / organisation sources listed in `lookups/sweep-thinktanks.csv` and picks up what those sources have published since it last ran. Run by `SWEEP-CYCLE.md` on **Day 1** of the nightly rotation; also runnable standalone from Claude Code on demand.
...
|---|---|---|---|---|
| **"run the thinktanks sweep"** | `sweep-thinktanks.csv` | `sweep/thinktanks/` | `URL, Title, Primary Focus` | the org's **news, blogs and papers** |
...
Any on-topic Africa-focused news, blogs, guides or reports are in scope.

**Deliberately broad, and not to be narrowed.** Think tanks are low-volume publishers: the net is wide by design and a thin return is the normal shape, not a defect. A narrow, cluster-style query of the kind the daily sweep runs against trade journals returns nothing here.
...
- **The list is its whole world.** This sweep touches **only the domains in `sweep-thinktanks.csv`**. Domain-scope every Exa query to the row's domain — this is also the admissibility firewall against content-mirrors and AI-synthesis blogs. Never run an un-scoped "latest Africa policy" query.
- **Origin screen — run `wiki/origin-screen.md`** anyway: a listed source can syndicate or republish, and a row can be added to a CSV faster than its origin is adjudicated. **Log any `inadmissible-origin` drop to `sweep/thinktanks/drop-log-YYYY-MM-DD.csv`** — nothing is discarded silently (`intake.md` §7).
- **Disjoint from the other sweeps.** The universal sweep of everything off the lists is **`SWEEP-DAILY-OFFLIST.md`**; the domain daily list is **`SWEEP-DAILY-LIST.md`**; newspapers and journals have their own sweeps. No double-searching.
...
Read `sweep-thinktanks.csv` fresh (it changes). For each row:

- **Domain-scope an Exa query** to the row's URL/domain for items published within `[window_start, window_end]`, **in the source's own language** (infer it from the domain — an English-only query misses Francophone or Lusophone organisations), seeking the org's **news, blogs and papers**. Prefer `web_search_exa` scoped to the domain; fetch bodies with `web_fetch_exa`.
- **Stage each hit flat into `new/`** as `new/YYYY-MM-DD-slug.md` (publication-date prefix), full verbatim body, best-effort frontmatter: `publisher` = the row's Title; `sweep_batch: thinktanks-YYYY-MM-DD` (this run's date); leave `place` for ingest to determine from content. Staged frontmatter is a head-start, validated at ingest — never a substitute for it.
...
Pruning organisations that never yield anything is a **manual job** — this sweep does not report on empties.
...
2. **Log one terse line** to `logs/log.md` — the **duration** and window, not what was captured or dropped (`raw/` and `logs/sweep-url_log.md` are that record):
   ...
   `thinktanks sweep — window <start>→<end>, took <Nm>, staged <N>.`
