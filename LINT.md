...
**8. Page bloat** — auto-resolve, batched. Over §8's length, or an append-log: trim or split per `CLAUDE.md` → *Structure*; shard an oversized index. Hub *Recent developments* exempt. Detect by where the date sits; measure per section.
...
**22. Acquisition names a held document** — auto-fix, nightly. `python scripts/lint-acquisition-held.py` (also `ACQUIRE.md` step 0); strike what it finds. `excerpt` on a structured extract or abstract is the complete record (§4).
