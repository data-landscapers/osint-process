...
| `full lint` | [LINT.md](../LINT.md) | The hygiene checks, numbered #1–#38 (numbers permanent). Acts and logs; surfaces only genuine contradictions. The whole-vault band is a nightly close step, or on demand. Thresholds in [operations.md](operations.md) §8. |
...
**Domestic-state record-building is retired** (R57, 2026-09-24): `BUDGET-EXTRACT`, `COUNTRY-BUDGET-BATCH` and `SWEEP-COUNTRY-BUDGET` are gone, OSINT mints no budget-line records, and finance compile writes no domestic budget export, so `outputs\budgets\` stays at its last-built content. Collection survives as `BUDGET-COLLECT` over `DOMESTIC-FINANCE-SWEEP`, both by name only. The non-state layer is live and nightly — `SWEEP-FINANCIERS`, `SWEEP-IATI`, `FINANCE-COMPILE` / `FINANCE-PAGES`, building `outputs\non-state-finance\`.
...
| `run domestic finance sweep for <country> <year>` | [DOMESTIC-FINANCE-SWEEP.md](../DOMESTIC-FINANCE-SWEEP.md) | By name only, or as `BUDGET-COLLECT.md` step 1. Budget documents, outturn and audit reports, ministerial statements for one country-year, staged to `new-budget/{ISO3}/{FY}/`. State in `sweep/domestic/`. |
...
| *(no caller since `BUDGET-EXTRACT.md` retired)* | [scripts/archive-budget-docs.py](../scripts/archive-budget-docs.py) | Archives extracted budget documents to `budget-archive/{ISO3}/{FY}/` from an extraction scope CSV; dry-run by default; `--verify`. Kept for re-archiving; `BUDGET-COLLECT.md` files documents itself. |
| *(`BUDGET-COLLECT.md`, between groups)* | [scripts/promote-budget-companions.py](../scripts/promote-budget-companions.py) | Catalogues held `budget-archive/` companions into `raw/`: full estimates volumes, appropriation and finance laws, outturn and audit reports, country by country in the order note 168 set. It copies rather than moves, and skips anything whose slug or URL is already held. Dry-run by default; `--write` applies. |
